...
import { Download, Link, Video, Youtube, Play, Loader2, Clipboard, Shield, History } from 'lucide-react';
  ...
  const handleFetchVideo = async () => {
    ...
    try {
      ...
      if (videoInfo.formats.length > 0) {
        ...
        // Log debug info
        console.log('Video Info Debug:', videoInfo.debug);
        toast({
          title: "Video Found!",
          description: `Fetched via ${videoInfo.debug?.usedClient || 'WEB'} (${videoInfo.formats.length} formats)`,
        });
      } else {
